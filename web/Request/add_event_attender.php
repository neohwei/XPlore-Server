...
if (isset($_POST[USER_UID]) && isset($_POST[USER_TOKEN]) && isset($_POST[EVENT_UID]) && isset($_POST[ATTENDER_STATUS])) {
    ...
    if($access) {
        ...
        $result = $db->addEventAttender($event_uid, $user_uid, $attender_status, $pdo);

        if ($result) {
            $response[TAG_ERROR] = false;
            $response[TAG_MESSAGE] = "User on event updated!";
            echo json_encode($response);
        } else {
            $response[TAG_ERROR] = true;
            $response[TAG_MESSAGE] = "User on event not updated ... Please try again!";
            echo json_encode($response);
        }

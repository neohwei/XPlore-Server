...
if (isset($_POST[USER_UID]) && isset($_POST[USER_TOKEN]) && isset($_POST[USER_GCM_ID])) {
    
    // receiving the post params
    $user_uid = $_POST[USER_UID];
    $user_token = $_POST[USER_TOKEN];
    $gcm_id = $_POST[USER_GCM_ID];
    ...
    if($access) {
        ...
        $result = $db->updateUserGCM($user_uid, $gcm_id, $pdo);
        
        if ($result) {
            $response[TAG_ERROR] = false;
            $response[TAG_MESSAGE] = "GCM Token updated!";
            echo json_encode($response);
        } else {
            $response[TAG_ERROR] = true;
            $response[TAG_MESSAGE] = "GCM Token not updated ... Please try again!";
            echo json_encode($response);
        }

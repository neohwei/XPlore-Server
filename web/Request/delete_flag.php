...
if (isset($_POST[USER_UID]) && isset($_POST[USER_TOKEN]) && isset($_POST[FLAG_UID])) {
    ...
    if($access) {
                ...
                $result = $db->deleteFlag($flag_uid, $pdo);

                if ($result) {
                    $response[TAG_ERROR] = false;
                    $response[TAG_MESSAGE] = "Flag deleted!";
                    echo json_encode($response);
                } else {
                    $response[TAG_ERROR] = true;
                    $response[TAG_MESSAGE] = "Flag not deleted ... Please try again!";
                    echo json_encode($response);
                }

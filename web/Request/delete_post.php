...
if (isset($_POST[USER_UID]) && isset($_POST[USER_TOKEN]) && isset($_POST[POST_UID])) {
    ...
    if($access) {
                ...
                $result = $db->deletePost($post_uid, $user_uid, $pdo);

                if ($result) {
                    $response[TAG_ERROR] = false;
                    $response[TAG_MESSAGE] = "Post deleted!";
                    echo json_encode($response);
                } else {
                    $response[TAG_ERROR] = true;
                    $response[TAG_MESSAGE] = "Post not deleted ... Please try again!";
                    echo json_encode($response);
                }

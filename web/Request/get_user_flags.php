    ...
    if($access) {
        ...
        if(isset($_POST[KEYWORD])){
        	$keyword = $_POST[KEYWORD];
        	 $flags = $db->searchUserFlags($user_uid, $search_user_uid, $keyword, $cursor, $pdo);
        }
        else{
        	 $flags = $db->getUserFlags($user_uid, $search_user_uid, $cursor, $pdo);
        } 

        if ($flags != false) {

				$response[TAG_FLAG] = $flags;
            ...
            $response[TAG_MESSAGE] = "Get flags failure!";
            echo json_encode($response);
        }
    }else{
        $response[TAG_ERROR] = TRUE;

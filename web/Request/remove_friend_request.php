...
if (isset($_POST[USER_UID]) && isset($_POST[USER_TOKEN]) && isset($_POST[SEARCH_USER_UID])) {
    ...
    if($access) {
        ...
        $result = $db->removeFriendRequest($user_uid, $search_user_uid, $pdo);
        
        if ($result) {
            $response[TAG_ERROR] = false;
            $response[TAG_MESSAGE] = "Request removed!";
            echo json_encode($response);
        } else {
            $response[TAG_ERROR] = true;
            $response[TAG_MESSAGE] = "Request not removed ... Please try again!";
            echo json_encode($response);
        }
